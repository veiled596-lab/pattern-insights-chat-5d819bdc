    ...
    if (header && !document.getElementById('filter-patterns-btn')) {
        ...
        filterBtn.onclick = function() {
            ...
            modal.innerHTML = 
                ...
                '<select id="activity-filter" multiple>' +
                [...new Set(AppState.currentPatterns.flatMap(p => p.occurrences.flatMap(o => o.activities)))].sort().map(a => 
                ...
                '<button class="primary-btn" onclick="' +
                    'const selected = Array.from(document.getElementById(\'activity-filter\').selectedOptions).map(o => o.value);' +
                    'if (selected.length === 0) { alert(\'Please select at least one activity\'); return; }' +
                    'const filtered = AppState.currentPatterns.filter(p => p.occurrences.some(o => selected.some(a => o.activities.includes(a)))).sort((a, b) => b.health_benefits_score - a.health_benefits_score);' +
                    'document.getElementById(\'pattern-log-list\').innerHTML = ' +
                    '(filtered.length === 0 ? \'<p class="no-patterns">No patterns found containing the selected activities.</p>\' : ' +
                    'filtered.map((p, i) => ' +
                    '\'<div class="pattern-item" onclick="showPatternDetails(\\\'\' + p.pattern_id + \'\\\')"><div class="pattern-header"><h4>\' + p.pattern_name.replace(/_/g, " ").replace(/\\\\b\\\\w/g, l => l.toUpperCase()) + \'</h4><span class="pattern-type">\' + p.pattern_type + \'</span></div><div class="pattern-stats"><span class="stat">📊 \' + p.avg_productivity + \'/10</span><span class="stat">😊 \' + p.avg_satisfaction + \'/10</span><span class="stat">🏥 \' + p.health_benefits_score + \'/100</span><span class="stat">🔁 \' + p.frequency + \' occurrences</span></div><div class="pattern-activities">\' + p.occurrences[0].activities.map(a => \'<span class="activity-tag">\' + a + \'</span>\').join(\'\') + \'</div></div>\'' +
                    ').join(\'\')) + ' +
                    '\'<div class="filter-header"><button class="secondary-btn" onclick="displayPatternLog()">Clear Filter</button></div>\';' +
                    'this.closest(\'.modal\').remove();">Apply Filter</button>' +
                '</div>' +
                '</div>' +
                '</div>';
